...
class TcpTransSpec extends Specification{
    ...
    def '测试发送并接收包含文件的 Request 和 Response'(){
        when:
        client.request=[action:'test',file:new File('data/'+'灰色鲨鱼.png').bytes]
        sleep(500)
        then:
    ...
    def '大文件测试'(){
        ...
        when:
        //大文件测试
        testServer.ctx.writeAndFlush([result:true,file:new File('data/'+'image.iso').bytes])
        def map = client.response
        then:
        map.result
        map.file.length==new File('data/'+'image.iso').size()
    }
